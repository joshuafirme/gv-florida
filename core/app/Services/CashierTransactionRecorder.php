...
class CashierTransactionRecorder
{
    ...
    public function recordRebooking(
        ...
        foreach ($slips->values() as $index => $slip) {
            $snapshot = $this->ticketSnapshot($ticket, $slip);
            $reference = (string) $slip->id;
            $snapshot['rebooking'] = array_merge($history, [
                'previous_seat' => $history['previous']['seats_by_reference'][$reference]
                    ?? $history['previous']['seats'][$index]
                    ?? null,
                'new_seat' => $history['new']['seats_by_reference'][$reference]
                    ?? $history['new']['seats'][$index]
                    ?? $slip->seat,
            ]);
            $this->store(
